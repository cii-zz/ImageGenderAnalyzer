...
class SimpleGenderDetector:
    ...
    def _init_models(self):
        ...
        try:
            ...
            self.ocr = CnOcr(
                rec_model_name='ch_PP-OCRv5_server',  # 识别模型
                det_model_name='ch_PP-OCRv5_det_server',  # 检测模型
    ...
    async def process_user(self, email, urls, session, semaphore):
        if not urls:
            return {'email': email, 'gender': '未知', 'method': -1, 'original_urls': ''}
    ...
    def _decide_gender(self, gender_results, method_results):
        ...
        for i, gender in enumerate(gender_results):
            if gender == most_common_gender:
                corresponding_methods.append(method_results[i])

        # 统计识别方式
        if corresponding_methods:
            method_counter = Counter(corresponding_methods)
            most_common_method = method_counter.most_common(1)[0][0]
        else:
            most_common_method = -1

        return most_common_gender, most_common_method
...
def main():
    ...
    method_stats = Counter(result_df['method'])
